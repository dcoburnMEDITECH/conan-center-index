...
class PdfiumConan(ConanFile):
    ...
    def _patch_gclient_stubs(self, src):
        ...
        # modules.gni stub — the real file uses path_exists() (unsupported by
        # gn/cci.20240611).  pdfium does not use clang modules, so disable them.
        save(self, os.path.join(src, "build", "config", "c++", "modules.gni"),
             textwrap.dedent("""\
                 # Stubbed by Conan — avoids path_exists() unsupported by older GN.
                 declare_args() {
    ...
    def _build_gn_args(self):
        ...
        if is_msvc(self) or self.settings.compiler == "gcc":
